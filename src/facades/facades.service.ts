...
export class FacadesService {
  ...
  constructor(@InjectModel(Facade) private facadeRepository: typeof Facade, private fileService: FilesService) { }

  async getAllFacades() {
    const facades = await this.facadeRepository.findAll({ include: { all: true } })
    return facades
  }
  ...
  async createFacade(dto: createFacadeDto, image: any) {
    ...
    const facade = await this.facadeRepository.create({
      name, link: fileName, height, width, areaWall, areaWindow,
    });
    return facade;
  }

  async deleteFacade(value: number) {
    const facade = await this.facadeRepository.findOne({ where: { id: value } });
